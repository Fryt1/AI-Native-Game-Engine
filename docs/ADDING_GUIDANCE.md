...
一份可复用的 Workflow 指引写在 `guidance/<name>.md`，也可以是一个目录
`guidance/<name>/`，用它的入口文档承载同一套内容。目录形态是给需要携带脚本、
schema、数据或模板的生命周期用的 —— 散文是**下限，不是上限**。用哪个形态，看这条
生命周期除了文字还需要交付什么；名字可以在两种形态之间升级，任务引用它的方式不变。
它给 Agent 的是宏观生命周期与不变式，不是固定的节点序列，也不含 Workflow 骨架。

**本仓库没有"配方包"这个概念。** 不存在 `recipes/`，不存在按 host/object/operation
组合维护的图或脚本。宿主与模型的版本前置条件一律声明在 `docs/DEPENDENCIES.md`，
不写在指引里。
...
guidance/<name>/        同一套内容，外加这条生命周期要交付的脚本 / schema / 数据
    README.md           入口文档，先读它
```

先决定用哪种形态：只需要散文就用单文件，需要携带可执行或可校验的东西就用目录。
两种形态的入口文档都按下面的骨架写，再按需增删小节。指引里要写：
...
1. 把新指引加入 `guidance/index.md` 的清单（目录形态写目录名，带尾斜杠）；
2. 把新指引的**入口文档**加入 `integrity_gate.py` 的 required 列表（目录形态写
   `<name>/README.md`）。

`tests/unit/skill/test_guidance_inventory.py` 会核对这两处与 `guidance/` 的实际内容；
漏登记任何一处都会失败，所以这一步不靠记性。
...
| 新增指引却不登记 | `index.md` 与完整性门都看不到它，等于没加 | 按上面第 4 步登记两处；有测试会拦 |
| 把"动态清单规则"整段复制进每份指引 | 同一段话在 5 份文件里各维护一份 | 规则说一次，指引只写这条生命周期特有的部分 |
